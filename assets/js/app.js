...
addEventOnElements($tabBtns, "keydown", function (e) {
  ...
  const $previousElement = this.prevElementSibling;
...
window.updateProfile = function (profileUrl) {
  ...
  fetchData(
      ...
      $profileCard.innerHTML = `
        <figure class="${
          type === "User" ? "avatar-circle" : "avatar-rounded"
            ...
            <img src="${avatar_url}" alt="${username}" class="img-cover" width="280" height="280" />
        </figure>
    ...
    <li class="stats-item">
        <span class="body">${public_repos}</span> Repositories
    </li>
    <li class="stats-item">
    ...
    <div class="footer">
    <p class="copyright">
        <script>document.write(new Date().getFullYear());</script> ntosh_webdevs_Inc. All rights reserved.
    </p>
...
const updateForkRepo = function () {
  $forkePanel.innerHTML = `
    <h2 class="sr-only">Forked respositories</h2>
  `;
      ...
      for(const item of data){
        ...
        $followerCard.innerHTML = `
          <figure class="avatar-circle img-holder">
            <img src="${avatar_url}&=64" width="56" height="56" loading="lazy" class="img-cover" alt='${username}'>
          </figure>
          <h3 class="card-title">${username}</h3>
          <button class="icon-btn" aria-label="Go to ${username} profile" onclick="updateProfile(\'${url}\')">
